-- Moderation audit trail. Run after 005_submission_moderation_rpc.sql.
...
create index if not exists moderation_audit_submission_idx
  on public.moderation_audit_log (submission_id, created_at desc);

create index if not exists moderation_audit_admin_idx
  on public.moderation_audit_log (admin_user_id, created_at desc);
...
begin
  ...
  insert into public.content_items (
    type, slug, title, category, description, details, date_text, published
  )
  values (
    submission.type, candidate_slug, submission.title, submission.category,
    submission.description, submission.details, submission.date_text, true
  )
  ...
  insert into public.moderation_audit_log (
    submission_id, admin_user_id, action, content_item_id
  )
  values (
    submission.id, auth.uid(), 'approved', result.id
  );
...
begin
  ...
  insert into public.moderation_audit_log (
    submission_id, admin_user_id, action
  )
  values (
    result.id, auth.uid(), 'rejected'
  );
