...
export default function DataDirectory({ title, eyebrow, description, items = [], detailType }) {
  ...
  return <ScrollView style={styles.root} contentContainerStyle={styles.page}>
    ...
    {filtered.length ? filtered.map((item) => <FeatureCard key={item.id} icon={item.category === 'Nature' ? '🌿' : item.category === 'Heritage' ? '🛕' : '📍'} title={item.title} text={item.description} onPress={detailType ? () => router.push({ pathname: '/details', params: { key: item.id, type: detailType } }) : undefined} />) : <View style={styles.empty}><Text style={styles.emptyTitle}>Nothing found</Text><Text style={styles.emptyText}>Try a different search term.</Text></View>}
  </ScrollView>;
}
