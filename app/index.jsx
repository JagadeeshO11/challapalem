  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.page}>
      <View style={[styles.nav, wide && styles.navWide]}>
        ...
        {wide && <View style={styles.links}>{[['Explore','/explore'],['Events','/events'],['Businesses','/businesses'],['Community','/community']].map(([label,path]) => <Pressable key={path} onPress={() => router.push(path)}><Text style={styles.link}>{label}</Text></Pressable>)}</View>}
      </View>
